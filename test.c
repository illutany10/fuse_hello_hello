...
/*static int custom_stat(int index, struct stat *buf){
...
static int hello_mknod(const char *path, mode_t mode, dev_t rdev)
{
	...
	if(index == -1){
		...
		mk_root();
		puts("mk_inode end - root been made");
		return 0;
	} else*/ {
...
/*static int hello_access(const char *path, int mask)
{
	...
	if (res == -1){
		puts("hello access end 1");
		return -errno;
	}
	puts("access end normally");
	return 0;
}*/
/*
static int hello_symlink(const char *from, const char *to)
{
	...
	res = symlink(from, to);
	if (res == -1){
		puts("hello symlink end (symlink = -1)");
		return -errno;
	}
	puts("symlink end normally");
	return 0;
}*/
...
/*static int hello_unlink(const char *path)
{
	...
	res = unlink(path);
	if (res == -1){
		puts("hello unlink end (unlink = -1)");
		return -errno;
	}
	puts("unlink end normally");
	return 0;
}*/
...
/*	int res;
	...
	if (res == -1){
		puts("hello utimens end (utimensat = -1)");
		return -errno;
	}
